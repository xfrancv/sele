...
for i = 1 : numel( dataSet )
    ...
    Exp1(i).Result(1).name  = 'SVORIMC+sele(linear)';
    Exp1(i).Result(1).fname = ['results/svorimc/' dataSet{i} '/conf_hinge1_linear_zmuv1_bs1000/results.mat'];

    Exp1(i).Result(end+1).name = 'SVORIMC+sele(quad)';
    Exp1(i).Result(end).fname  = ['results/svorimc/' dataSet{i} '/conf_hinge1_quad_zmuv1_bs2000/results.mat'];
    
    Exp1(i).Result(end+1).name  = 'SVORIMC+regression(linear)';
    Exp1(i).Result(end).fname   = ['results/svorimc/' dataSet{i} '/conf_regression_linear_zmuv1/results.mat'];
    
    Exp1(i).Result(end+1).name  = 'SVORIMC+regression(quad)';
    Exp1(i).Result(end).fname   = ['results/svorimc/' dataSet{i} '/conf_regression_quad_zmuv1/results.mat'];
