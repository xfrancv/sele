function [W,S,C]= oca(Data,riskls,lambda, Aineq, bineq, C, Opt )
% OCA Optimized cutting plane algorithm.
%
% Synopsis: 
%  [W,Stat,Cpm] = oca( Data, riskls, lambda )
%  [W,Stat,Cpm] = oca( Data, riskls, lambda, Aineq, bineq )
%  [W,Stat,Cpm] = oca( Data, riskls, lambda, Aineq, bineq, Cpm )
%  [W,Stat,Cpm] = oca( Data, riskls, lambda, Aineq, bineq, Cpm, Options )
%
% Description:
%  This solver minimizes a quadraticaly regularized convex risk
%
%    F(W) = 0.5*lambda*W'*W + R( W )
%
%  subject to
%
%    Aineq*W >= bineq
%
%  where risk(data,W) is an arbitrary convex function of W. OCA requires 
%  a procedure "riskls" that perfroms a line-search on the primal objective 
%  and uses the solution to define a new cut. The "riskls" function 
%  has the syntax
%    [R,subgrad,Fbsf,Wnew] = riskls( Data, Wd, Wbsf, lambda )
%
%  The riskls first solves 
%    k = min_{k>= 0 } F(Wbsf*(1-k)+k*Wd)  s.t. Aineq*(Wbsf*(1-k)+k*Wd) >= bineq
%  and subsequently it computes
%    Wnew = Wbsf*(1-k) + Wd*k 
%    Fbsf = F(Wbsf*(1-k)+k*Wd) 
%    Wc   = Wnew*(1-mu) + Wd*MU
%    R  ... value of the risk at Wc
%    subgrad ... subgradient of risk at Wc
%
%  where MU is a hyper-parameter whose good value is MU=0.1.
%
% Inputs:
%  Data   [anything]       Parameters of the risk function.
%  riskls [function]       Pointer at a function performing lineasearch and 
%                          riskevaluation.
%  lambda [1x1]            Regularization parameter.
%  Aineq  [nConstr x nDim] linear constraitns; default [] meaning no constr
%  bineq  [nConstr x 1]    --//-- 
...
%  Cpm [struct]  Cutting plane modle of the risk function.
%
    ...
    %% start from given cutting plane model or if not given allocate memory for 
    ...
    if nargin >= 6 & ~isempty( C )
        ...
        S.nD               = C.nCp + C.nConstr;
        t0                 = cputime;
        C.H(1:S.nD,1:S.nD) = C.A(:,1:S.nD)'*C.A(:,1:S.nD);
