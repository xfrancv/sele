function [net,stats] = cnn_train_dag(net, imdb, getBatch, Meta, varargin)
...
for epoch=start+1:opts.numEpochs
  ...
  if numel(opts.gpus) <= 1
    ...
    if ~evaluateMode
      saveState(modelPath(epoch), net, state, Meta, opts) ;
    end
...
% -------------------------------------------------------------------------
function saveState(fileName, net_, state, Meta, opts)
% -------------------------------------------------------------------------
net = net_.saveobj() ;
save(fileName, 'net', 'state', 'Meta', 'opts') ;
