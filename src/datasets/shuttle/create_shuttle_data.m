...
%%
if ~exist( outFile )
    system('wget --no-check-certificate https://archive.ics.uci.edu/ml/machine-learning-databases/statlog/shuttle/shuttle.tst');
    system('wget --no-check-certificate http://archive.ics.uci.edu/ml/machine-learning-databases/statlog/shuttle/shuttle.trn.Z');
    system('uncompress shuttle.trn.Z');
end


%%
[x1,x2,x3,x4,x5,x6,x7,x8,x9,y] = textread('shuttle.trn', '%f%f%f%f%f%f%f%f%f%f', 'delimiter',',');

X1 = [x1 x2 x3 x4 x5 x6 x7 x8 x9]';
Y1 = y(:);

[x1,x2,x3,x4,x5,x6,x7,x8,x9,y] = textread('shuttle.tst', '%f%f%f%f%f%f%f%f%f%f', 'delimiter',',');
